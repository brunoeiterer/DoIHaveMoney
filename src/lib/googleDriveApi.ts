export async function createBudgetSpreadsheet(budgetName: string, folderId: string, accessToken: string) {
    ...
    const sheetsRes = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}:batchUpdate`, {
        method: 'POST',
        headers: {
            'Authorization': `Bearer ${accessToken}`,
            'Content-Type': 'application/json'
...
export async function addExpenseToSheet(spreadsheetId: string, expense: any, token: string) {
    const res = await fetch(
